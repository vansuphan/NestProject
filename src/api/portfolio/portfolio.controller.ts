...
export class PortfolioController extends BaseController<Portfolio> {
  ...
  @Get()
  async findAll(@Param("page") page: number, @Param("limit") limit: number): Promise<CorePaginateResult<Portfolio>> {
    return super.findAll(page, limit);
  }

  @UseGuards(AuthGuard())
  @Patch(":id")
  async update(@Body() updates: UpdatePortfolioDto, @Param("id") id: number): Promise<Portfolio> {
